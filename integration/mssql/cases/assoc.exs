...
defmodule Ecto.Integration.AssocTest do
  ...
  test "has_many through-through assoc leading" do
    ...
    query =
      Ecto.assoc([p1, p2], :comments_authors_permalinks) |> order_by([p], p.url)

    assert [^pl2, ^pl1] = TestRepo.all(query)

    # Dynamic through
    query =
      Ecto.assoc([p1, p2], [:comments, :author, :permalink])
      |> order_by([p], p.url)
  ...
  test "has_one changeset assoc (on_replace: :update)" do
    ...
    assert_raise RuntimeError,
                 ~r"you are only allowed\sto update the existing entry",
                 fn ->
                   post
                   |> Ecto.Changeset.change()
                   |> Ecto.Changeset.put_assoc(:update_permalink, %Permalink{
                     url: "4"
                   })
                 end
    ...
    assert_raise RuntimeError,
                 ...
                 fn ->
                   ...
                   |> Ecto.Changeset.put_assoc(
                     :update_permalink,
                     TestRepo.insert!(%Permalink{url: "5"})
                   )
                 end
  ...
  test "many_to_many changeset assoc" do
    ...
    assert [1] ==
             TestRepo.all(from(j in "posts_users", select: count(j.post_id)))
  ...
  test "has_many assoc on delete nilifies all" do
    ...
    author_ids =
      Comment |> TestRepo.all() |> Enum.map(fn comment -> comment.author_id end)

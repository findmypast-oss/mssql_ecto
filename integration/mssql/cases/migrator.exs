...
defmodule Ecto.Integration.MigratorTest do
  ...
  test "schema migration" do
    up(PoolRepo, 20_100_906_120_000, GoodMigration, log: false)

    [migration] = PoolRepo.all(SchemaMigration)
    assert migration.version == 20_100_906_120_000
    assert migration.inserted_at
  end

  test "migrations up and down" do
    assert migrated_versions(PoolRepo) == []
    assert up(PoolRepo, 20_100_906_120_000, GoodMigration, log: false) == :ok

    assert migrated_versions(PoolRepo) == [20_100_906_120_000]

    assert up(PoolRepo, 20_100_906_120_000, GoodMigration, log: false) ==
             :already_up

    assert migrated_versions(PoolRepo) == [20_100_906_120_000]

    assert down(PoolRepo, 21_100_906_120_000, GoodMigration, log: false) ==
             :already_down

    assert migrated_versions(PoolRepo) == [20_100_906_120_000]
    assert down(PoolRepo, 20_100_906_120_000, GoodMigration, log: false) == :ok
    assert migrated_versions(PoolRepo) == []
  end

  test "bad migration" do
    assert catch_error(
             up(PoolRepo, 20_100_906_120_000, BadMigration, log: false)
           )
  end

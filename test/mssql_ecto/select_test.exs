defmodule MssqlEcto.SelectTest do
  ...
  test "distinct" do
    ...
    query =
      Schema |> distinct([r], desc: r.x) |> select([r], {r.x, r.y}) |> normalize
    ...
    query =
      Schema |> distinct([r], true) |> select([r], {r.x, r.y}) |> normalize

    assert SQL.all(query) ==
             ~s{SELECT DISTINCT s0."x", s0."y" FROM "schema" AS s0}

    query =
      Schema |> distinct([r], false) |> select([r], {r.x, r.y}) |> normalize
  ...
  test "limit and offset" do
    ...
    query =
      Schema |> offset([r], 5) |> limit([r], 3) |> select([], true) |> normalize
  ...
  test "literals" do
    ...
    query =
      "schema" |> where(foo: <<0, ?a, ?b, ?c>>) |> select([], true) |> normalize
  ...
  test "or_having" do
    query =
      Schema |> or_having([p], p.x == p.x) |> select([], true) |> normalize
